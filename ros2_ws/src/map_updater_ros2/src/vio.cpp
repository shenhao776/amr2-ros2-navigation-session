...
void VIOManager::insertPointIntoVoxelMap(std::shared_ptr<VisualPoint> pt_new) {
  ...
  if (iter != feat_map.end()) {
    ...
    auto ot = std::make_shared<VOXEL_POINTS>(0);
...
void VIOManager::getWarpMatrixAffineHomography(
    ...
    const V3D& normal_ref, const SE3d& T_cur_ref, const int level_ref,
    Matrix2d& A_cur_ref) {
...
void VIOManager::retrieveFromVisualSparseMap(
    ...
    const unordered_map<VOXEL_LOCATION, std::shared_ptr<VoxelOctoTree>>&
        plane_map) {
      ...
      for (const auto& it : rays_with_sample_points[i]) {
        ...
        if (corre_feat_map != feat_map.end()) {
          ...
          if (iter != plane_map.end()) {
            auto current_octo = iter->second->find_correspond(sample_point_w);
  ...
  for (int i = 0; i < length; ++i) {
    if (grid_num[i] == TYPE_MAP) {
      ...
      auto pt = retrieve_voxel_points[i];
      ...
      if (normal_en) {
        ...
        if (iter_warp != warp_map.end()) {
          ...
          auto ot = std::make_shared<Warp>(search_level, A_cur_ref_zero);
...
void VIOManager::generateVisualMapPoints(cv::Mat img,
  ...
  for (size_t j = 0; j < visual_submap->add_from_voxel_map.size(); ++j) {
  ...
  for (int i = 0; i < length; ++i) {
    if (grid_num[i] == TYPE_POINTCLOUD)  // && (scan_value[i]>=50))
    {
      ...
      auto pt_new = std::make_shared<VisualPoint>(pt);

      Vector3d f = cam->cam2world(pc);
      auto ftr_new = std::make_shared<Feature>(pt_new, patch, pc, f,
                                               new_frame_->T_f_w_, 0);
      ftr_new->img_ = img;
...
void VIOManager::updateVisualMapPoints(cv::Mat img) {
  ...
  for (int i = 0; i < total_points; ++i) {
    ...
    std::vector<float> patch_temp(patch_size_total);
    getImagePatch(img, pc, patch_temp.data(), 0);
    ...
    if (add_flag) {
      ...
      Vector3d f = cam->cam2world(pc);
      auto ftr_new =
          std::make_shared<Feature>(pt, patch_temp, pc, f, new_frame_->T_f_w_,
...
void VIOManager::updateReferencePatch(
  ...
  for (size_t i = 0; i < visual_submap->voxel_points.size(); ++i) {
    ...
    if (iter != plane_map.end()) {
      auto current_octo = iter->second->find_correspond(p_w);
      if (current_octo->plane_ptr_->is_plane_) {
        VoxelPlane& plane = *current_octo->plane_ptr_;
...
void VIOManager::projectPatchFromRefToCur(
  ...
  for (size_t i = 0; i < visual_submap->voxel_points.size(); ++i) {
    ...
    if (pt->is_normal_initialized_) {
      ...
      for (size_t i = 0; i < pixel_warp_matrix.size(); ++i) {
        vector<pixel_member> pixel_warp_row = pixel_warp_matrix[i];
        for (size_t j = 0; j < pixel_warp_row.size(); ++j) {
...
void VIOManager::precomputeReferencePatches(int level) {
  // double t1 = omp_get_wtime(); // 标记未使用变量
  if (total_points == 0) return;
  MD(1, 2) Jimg;
  MD(2, 3) Jdpi;
...
#pragma omp parallel for reduction(+ : error, n_meas)
    ...
    for (int i = 0; i < total_points; ++i) {
      ...
      // LOG_ERROR_F("inv_ref_expo: %.3lf, state->inv_expo_time: %.3lf\n",
...
void VIOManager::processFrame(
  ...
  new_frame_ = std::make_unique<Frame>(cam, img);
  updateFrameState(*state);
