...
void loadVoxelConfig(rclcpp::Node* node, VoxelMapConfig& voxel_config) {
  ...
  std::vector<int64_t> default_layer_init = {5, 5, 5, 5, 5};
  node->declare_parameter("lio.layer_init_num", default_layer_init);
  std::vector<int64_t> temp_layer_init;
  node->get_parameter("lio.layer_init_num", temp_layer_init);
  voxel_config.layer_init_num_.clear();
  for (auto v : temp_layer_init) voxel_config.layer_init_num_.push_back((int)v);
...
void VoxelOctoTree::init_octo_tree() {
  if ((int)temp_points_.size() > points_size_threshold_) {
    init_plane(temp_points_, plane_ptr_);
    if (plane_ptr_->is_plane_ == true) {
      octo_state_ = 0;
      if ((int)temp_points_.size() > max_points_num_) {
...
void VoxelOctoTree::cut_octo_tree() {
  ...
  for (size_t i = 0; i < temp_points_.size(); ++i) {
    ...
    if (leaves_[leafnum] == nullptr) {
      leaves_[leafnum] = std::make_shared<VoxelOctoTree>(
          max_layer_, layer_ + 1, layer_init_num_[layer_ + 1], max_points_num_,
          planer_threshold_);
  ...
  for (uint i = 0; i < 8; ++i) {
    if (leaves_[i] != nullptr) {
      if ((int)leaves_[i]->temp_points_.size() >
...
std::shared_ptr<VoxelOctoTree> VoxelOctoTree::find_correspond(
    Eigen::Vector3d pw) {
...
void VoxelMapManager::StateEstimation(StatesGroup& state_propagat) {
  ...
  for (int iterCount = 0; iterCount < config_setting_.max_iterations_;
    ...
    auto world_lidar = std::make_shared<pcl::PointCloud<pcl::PointXYZI>>();
    ...
    if (effct_feat_num_ > 0) {
      ...
      for (int i = 0; i < effct_feat_num_; ++i) {
        ...
        // ▼▼▼ 改动:理论上用state_propagat作为线性化点会更快才对 ▼▼▼
      ...
      // 注意：这里的 state_.cov 仍然是来自先验 state_propagat.cov，这是正确的。
      // 卡尔曼增益的计算应该基于更新前的协方差。
      ...
      if (!EKF_stop_flg &&
        ...
        // 最终的协方差更新，使用的是先验协方差 state_propagat.cov
      ...
      cout << "[ LIO ] No effective features. Skipping EKF update." << endl;
      EKF_stop_flg = true;
    }
...
void VoxelMapManager::BuildVoxelMap() {
  ...
  for (uint i = 0; i < plsize; ++i) {
    ...
    if (iter != voxel_map_.end()) {
      ...
      auto octo_tree = std::make_shared<VoxelOctoTree>(
...
// in src/voxel_map.cpp

void VoxelMapManager::build_single_residual(
    pointWithVar& pv, const std::shared_ptr<VoxelOctoTree> current_octo,
...
void VoxelMapManager::GetUpdatePlane(
    const std::shared_ptr<VoxelOctoTree> current_octo,
    const int pub_max_voxel_layer, std::vector<VoxelPlane>& plane_list) {
