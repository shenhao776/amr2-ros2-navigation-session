...
class VisualPoint : boost::noncopyable {
 public:
  ...
  list<std::shared_ptr<Feature>>
      obs_;                     //!< Reference patches which observe the point.
  ...
  ~VisualPoint();
  void findMinScoreFeature(const Vector3d& framepos,
  ...
  bool getCloseViewObs(const Vector3d& pos, std::shared_ptr<Feature>& obs,
